...
func getOverlayUpperdir(lower, upper []mount.Mount) (string, error) {
	...
	if len(lower) == 1 && len(upper) == 1 {
		// We only support single mount configuration as of now.
		upperM, lowerM := upper[0], lower[0]
		...
		case "overlay":
			...
			lowerlayers = getOverlayLayers(lowerM)
		default:
		...
		upperlayers := getOverlayLayers(upperM)

		// Check if the diff directory can be determined
		if len(upperlayers) != len(lowerlayers)+1 {
			return "", fmt.Errorf("cannot determine diff of more than one upper directories")
		}
	...
	if upperdir == "" {
		return "", fmt.Errorf("cannot determine upperdir from mount option")
	}
	return upperdir, nil
}

// getOverlayLayers returns all layer directories of an overlayfs mount.
func getOverlayLayers(m mount.Mount) []string {
	var u string
	var uFound bool
