...
func TestSharingCompressionVariant(t *testing.T) {
	...
	do(func(testCase testCaseSharingCompressionVariant) {
		...
		testSharingCompressionVariant(t, ctx, co, testCase)
	...
	do(func(testCase testCaseSharingCompressionVariant) {
		eg.Go(func() error {
			...
			testSharingCompressionVariant(t, egctx, co, testCase)
			return nil
		})
	})
...
func testSharingCompressionVariant(t *testing.T, ctx context.Context, co *cmOut, testCase testCaseSharingCompressionVariant) {
	var (
		cm              = co.manager
	...
	test := func(customized bool) {
		...
		for _, compressionType := range testCase.aVariants {
			remotes, err := aRef.GetRemotes(ctx, true, compression.New(compressionType).SetForce(true), false, nil)
			require.NoError(t, err)
		...
		require.NotEqual(t, "", bDesc.Digest, "compression B must be choosen from the variants of A")
		bRef, err := cm.GetByBlob(ctx, bDesc, nil, descHandlers)
		require.NoError(t, err)
		defer bRef.Release(ctx)
		for _, compressionType := range testCase.bVariants {
			remotes, err := bRef.GetRemotes(ctx, true, compression.New(compressionType).SetForce(true), false, nil)
			require.NoError(t, err)
			require.Equal(t, 1, len(remotes))
			require.Equal(t, 1, len(remotes[0].Descriptors))
		}
...
func ensurePrune(t *testing.T, ctx context.Context, cm Manager, pruneNum, maxRetry int) {
	sum := 0
